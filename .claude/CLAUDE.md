...
<!-- corrections.md is created on demand by the learning loop; add the import when it exists. -->

> **Note**: this repository also has a hand-maintained `CLAUDE.md` at the root covering
> architecture, the two sync loops, printing, migrations, and known warts. Claude Code loads
> both files. The root one is the authority on how the app actually behaves; the blueprints
> above are the AI-DLC view of the same system. Keep them from contradicting each other.
...
2. Project content is imported above (blueprints); design docs at `.aidlc/specs/{feature}/`
