...
## อัปเดต — หลัง task 6.2 (2026-09-24)

publish หน้า login ที่แปลงเป็น NativeWind แล้ว
group **`be660547-5f27-41fc-9bc3-60cf8b6787f5`** (commit `d4a44d5`)

**จุดย้อนกลับเปลี่ยนเป็น `70ce04e7`** — ไม่ใช่ `55bbdb85` อีกต่อไป

```bash
npx eas update:republish --group 70ce04e7-3b6a-4d2d-89ff-d38364636626
```

เหตุผลที่ใช้ `70ce04e7`: เป็นสถานะที่พิสูจน์แล้วว่าทำงานได้จริง (task 3.2) และ**มี toolchain อยู่แล้ว**
การย้อนกลับไปตัวนี้จึงถอนเฉพาะการเปลี่ยน UI ของหน้า login ไม่ได้ถอน NativeWind ทั้งก้อน
ทำให้แก้แล้วส่งใหม่ได้เร็วโดยไม่ต้องตั้ง toolchain ซ้ำ

*(`55bbdb85` ยังใช้ได้ถ้าต้องถอย NativeWind ออกทั้งหมด แต่ไม่ใช่ตัวเลือกแรก)*

---

## อัปเดต — ย้อนกลับแล้วกู้คืน (2026-09-24T06:55:49Z)

ลำดับเหตุการณ์บน production:
1. `be660547` — หน้า login แบบ NativeWind (task 6.2)
2. `cd01284b` — ผู้ใช้ republish `70ce04e7` ย้อนกลับไป toolchain เปล่า
3. **`ceaab146`** — republish `be660547` กลับคืน ← **สถานะปัจจุบัน**

**เหตุผลที่ย้อนกลับรอบนั้น**: ผู้ใช้เข้าใจว่าการที่หน้า login ไม่เปลี่ยนแปลงคือความผิดพลาด
ความจริงคือมันเป็นเกณฑ์ความสำเร็จของ scope `refactor` ที่ตั้งไว้ตั้งแต่ต้น

**บทเรียน**: คำขอเดิมคือ "ปรับ style ให้มาใช้ tailwind คิดว่าเหมาะกับ uxui มากกว่า" ซึ่งมีสองส่วน —
วิธี (tailwind) กับเป้าหมาย (uxui ดีขึ้น) การจัด scope เป็น `refactor` ตอบเฉพาะส่วนแรก
และไม่มีใครชี้ความขัดแย้งนี้ตอน context gate **งานออกแบบใหม่จึงเป็น feature แยกต่างหาก**

จุดย้อนกลับสำหรับงานถัดไป: **`ceaab146-b72e-4a07-92e2-92620929ff42`**
