...
- The `if (response.status !== 200)` branch in `CheckInSyncManager.js` calls `await response.text()`, which axios responses do not have. axios throws on non-2xx so the branch is usually dead, but a `201`/`204` would hit it and fail with a TypeError that gets logged as an upload error.
- Temp files from `ImageManipulator` are never deleted, and `photo_path` points into the ImagePicker cache, which Android may evict. A long-queued row can silently lose its photo with no `error_msg` set.
- `markCheckInAsSynced()` does not clear `error_msg`, so a row that failed once and later succeeded still carries the stale message in the Settings export.
- The success modal in `scan.js` resets `isSubmitting` from its OK button, but `onRequestClose` (Android back) only hides it — dismissing that way leaves the save button disabled.
